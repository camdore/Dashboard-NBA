...
# Image of the court from the web
...
#############################################Data processing##########################################################################
    ...
    # new data frame for position and points plot
    Position_points = select(nba18, Pos, X3P, X2P, FT)
    colnames(Position_points) <- c("Position","3 Points","2 Points","Free Throws")
    Position_points = aggregate(.~Position, data = Position_points, FUN = sum) #sum only points of players in the same position
    
    
    # Read Lebron James Geolocalised data
    ...
    Lebronjames_shots_m$GAME_DATE <- year(ymd(Lebronjames_shots_m$GAME_DATE))
        ...
        Lebronjames_shots_md$Points <- rep(1,1869)              # every lines is a point 
        Lebronjames_shots_md = select(Lebronjames_shots_md,GAME_DATE,SHOT_TYPE,Points )
        Lebronjames_shots_md_tot = aggregate(Lebronjames_shots_md$Points, list(Lebronjames_shots_md$SHOT_TYPE,Lebronjames_shots_md$GAME_DATE), FUN = sum) #sum all points
        ...
        Lebronjames_shots_ab$Points <- rep(1,1249)              # every lines is a point 
        Lebronjames_shots_ab = select(Lebronjames_shots_ab,GAME_DATE,SHOT_TYPE,Points )
        Lebronjames_shots_ab_tot = aggregate(Lebronjames_shots_ab$Points, list(Lebronjames_shots_ab$SHOT_TYPE,Lebronjames_shots_ab$GAME_DATE), FUN = sum) #sum all points
    ...
    # Lebron James geolocalised shots over the years
...
#############################field goals statistics##################################################################################   
    ...
    output$distPlot <- renderPlot({
        ...
        ggtitle("Best scorers of the season")

         
    })
#############################field goals statistics insight###########################################################################
...
#############################Player's positions and field goals########################################################################
    output$plot1 <- renderPlot({ 
      ...
      ggplot(Position_points_longer, aes(x = Position , y  = Points, fill= Type_of_point  )) + 
        geom_bar(stat = "identity" )+
        theme(text = element_text(size = 20))+
        ggtitle("Positions and field goals")
...
#############################Field goals################################################################################################
    ...
    output$plot2 <- renderPlot({ 
      ...
      ggplot(Lebronjames_shots_tot_fin , aes(x = Year , y  = Points, color= `Type_of_shot`, linetype=`Type_of_shot`)) + 
        ...
        ylab("Points")+
        theme(text = element_text(size = 20))+
        ggtitle("Field Goals over the years (Data from 2010 to 2012 seasons are absents)")
...
############################Player's age and position####################################################################################
    ...
    output$plottt <- renderPlot({
      ...
      ggplot(nba18, aes(x=Age, fill=Pos)) + geom_histogram(bins = 23) + theme(text = element_text(size = 20)) + 
        ...
        theme(text = element_text(size = 20))+
        ggtitle("Age and position")
    })
    

############################Geolocalisation of shots#####################################################################################    
    
    output$plot3 <- renderPlot({ 
      
      season_shots = Lebron_geo_shots[Lebron_geo_shots$GAME_DATE >= paste0(input$shoot_year,-10) & Lebron_geo_shots$GAME_DATE <=paste0(input$shoot_year+1,-4) , ] # season start in october and ends in april
      ...
      ggplot(season_shots, aes(x=LOC_X, y=LOC_Y)) + 
        ...
        theme(text = element_text(size = 20))+
          ...
          ggtitle("Geolocalised Field Goals (Data from 2010 to 2012 are missing)")}
